...
For this you need to provision your GKE cluster with the `--enable-ip-aliases` parameter and then add the `cloud.google.com/neg: '{"ingress": true}'` annotation on your `Service` (even if you expose it via an `Ingress`). The recommendation is to explicitly set this annotation where you need it, even if in some cases it will be [applied by default under certain conditions](https://cloud.google.com/kubernetes-engine/docs/concepts/ingress#container-native_load_balancing). You could also [find the associated requirements, restrictions and limitations](https://cloud.google.com/kubernetes-engine/docs/concepts/container-native-load-balancing#requirements) information about that feature. You could then see the associated backend service generated by running this command: `gcloud compute backend-services list`.

Other you could now leverage [in addition to the load balancer is Cloud Armor or Cloud CDN](https://cloud.google.com/blog/products/networking/using-cloud-armor-and-cloud-cdn-with-your-google-load-balancer). Here is how to [Configure Ingress features through BackendConfig parameters](https://cloud.google.com/kubernetes-engine/docs/how-to/ingress-features#configuring_ingress_features_through_backendconfig_parameters).
...
Another step now is, what if you would like a managed Istio service? Here comes [Anthos Service Mesh (ASM)](https://cloud.google.com/service-mesh/docs/overview)! For this you need an Anthos subscription but it will help you managed ASM (i.e. managed Istio) with your GKE clusters
- ASM and Istio tutorial: https://cloud.google.com/solutions/exposing-service-mesh-apps-through-gke-ingress
https://cloud.google.com/solutions/exposing-service-mesh-apps-through-gke-ingress#anthos-service-mesh_4
https://cloud.google.com/service-mesh/docs/upgrading-gke
https://cloud.google.com/service-mesh/docs/supported-features

Here are few resources to help you navigate through the capabilities of ASM:
...
Here are few resources to help you navigate through the capabilities of Traffic Director:
...
- [Traffic Director setup for Google Kubernetes Engine Pods with automatic Envoy injection](https://cloud.google.com/traffic-director/docs/set-up-gke-pods-auto)
...
That's a wrap! Hope you enjoyed this blog article and hopefully you will be able to leverage such important features transparently if enabled ;)
